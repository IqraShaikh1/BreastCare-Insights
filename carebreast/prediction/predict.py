...
def visualize_gradcam(image_path, gradcam, alpha=0.5, threshold=0.3):
    ...
    heatmap_path = os.path.join(settings.MEDIA_ROOT, 'heatmap.png')  # Correct usage
...
def predict_cancer(image_path, threshold=0.5, visualize=False):
    ...
    heatmap_path = None  # Initialize heatmap path
    if visualize:
        gradcam = apply_gradcam(model, input_tensor, target_layer)
        heatmap_path = visualize_gradcam(image_path, gradcam)  # Update to get heatmap path
    ...
    if prediction.item() >= threshold:
        return "Cancer detected", heatmap_path
    else:
        return "No cancer detected", heatmap_path
